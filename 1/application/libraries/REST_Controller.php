	...
	*/
	...
	public function _remap($object_called, $arguments)
	{
		...
		if (config_item('rest_enable_keys') AND $use_key AND $this->_allow === FALSE)
		{
			...
			$this->response(array('status' => false, 'error' => '缺少参数：appkey'), 403);
		}
	...
	{
		global $CFG;
		header('Access-Control-Allow-Origin:*');//http://www.cnblogs.com/Darren_code/p/cors.html 
	...
	protected function _log_request($authorized = FALSE)
	{
		foreach ($this->_args as $key => $value) {
			//var_dump($this->_args);
			//$this->_args[$key] = urlencode($value);//Changed by bryan
			$this->_args[$key] = $value;
		}
		return $this->rest->db->insert(config_item('rest_logs_table'), array(
					'uri' => $this->uri->uri_string(),
					'method' => $this->request->method,
					'params' => $this->_args ? (config_item('rest_logs_json_params') ? urldecode(json_encode($this->_args)) : serialize($this->_args)) : null,
